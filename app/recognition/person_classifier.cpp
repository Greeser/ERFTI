...
PersonClassifier::PersonClassifier(const std::string prototxt_path, const std::string model_path, const int gpuid)
{
    if (gpuid < 0)
    {
...
std::vector<float> PersonClassifier::Detect(const cv::Mat &face)
{
    cv::Mat normalized(target_, CV_32FC1);
    cv::resize(face, normalized, target_);
...
void PersonClassifier::setOutput(const int output)
{
    number_of_features_ = output;
}

void PersonClassifier::setInput(const cv::Size target)
{
    target_ = target;
}
